...
def init_db():
    conn = sqlite3.connect('busines_orders.db')
    cur = conn.cursor()
...
def comlete_order():
    ...
    if selected_item:
        ...
        messagebox.showwarning("Предупреждение", "Выберите заказ для выполнения")
...
complete_button = tk.Button(app, text="Выполнить заказ", command=comlete_order)
